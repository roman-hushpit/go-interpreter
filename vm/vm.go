...
type VM struct {
	...
	stack   []object.Object
	sp      int //Always points to the next free slot
...
func (vm *VM) Run() error {
	...
	for vm.currentFrame().ip < len(vm.currentFrame().Instructions())-1 {
		...
		case code.OpGetFree:
			freeIndsex := code.ReadUint8(ins[ip+1:])
			vm.currentFrame().ip += 1

			currentClosure := vm.currentFrame().cl

			err := vm.push(currentClosure.Free[freeIndsex])
			if err != nil {
				return err
			}
...
func (vm *VM) buildHash(startIndex int, endIndex int) (object.Object, error) {
	...
	for i := startIndex; i < endIndex; i += 2 {
		...
		if !ok {
			return nil, fmt.Errorf("unhashable as hash key: %s", key.Type())
		}
...
func (vm *VM) executeHashIndex(hash, index object.Object) error {
	...
	if !ok {
		return fmt.Errorf("unhashable as hash key: %s", index.Type())
	}
